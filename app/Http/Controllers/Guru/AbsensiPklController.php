...
class AbsensiPklController extends Controller
{
    public function index()
    {
        ...
        $absensi = AbsensiPkl::with(['pengajuanPkl.siswa.user', 'pengajuanPkl.tempatPkl'])
            ->whereHas('pengajuanPkl', function ($q) use ($guru) {
        ...
        return view('guru.absensi.index', compact('absensi', 'periodes', 'selectedPeriodeId'));
    }
    ...
    public function export(\Illuminate\Http\Request $request)
    {
        ...
        // Get all students bimbingan for this guru
        $students = Siswa::whereHas('pengajuanPkl', function ($q) use ($guru, $selectedPeriodeId) {
            $q->where('guru_id', $guru->id)
        ...
        $filename = "Rekap_Absensi_Siswa_Bimbingan_" . str_replace([' ', '/'], '_', $periodeName) . "_{$bulanNama}_{$year}.xls";
