<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('guru.hubungi-admin.index') }}" class="p-2 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-surface-600 transition-colors">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <h2 class="font-extrabold text-lg md:text-2xl text-surface-900 tracking-tight">Kirim Pesan ke Admin</h2>
        </div>
    </x-slot>
    ...
    <div class="py-6 md:py-10 animate-fade-in">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="card-premium p-6 sm:p-8">
                <form action="{{ route('guru.hubungi-admin.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <label class="form-label">Subjek Pesan</label>
                            <input type="text" name="subjek" value="{{ old('subjek') }}" class="form-input-premium mt-1.5" placeholder="Contoh: Pertanyaan Sistem Penilaian / Error input Jurnal" required>
                        ...
                        <div>
                            ...
                            <select name="kategori" class="form-input-premium mt-1.5" required>
                                ...
                                <option value="penilaian" {{ old('kategori') == 'penilaian' ? 'selected' : '' }}>Input Nilai Akhir</option>
                                <option value="jurnal" {{ old('kategori') == 'jurnal' ? 'selected' : '' }}>Validasi Jurnal Siswa</option>
                                <option value="laporan" {{ old('kategori') == 'laporan' ? 'selected' : '' }}>Review Laporan PKL</option>
                        ...
                        <div>
                            <label class="form-label">Isi Pesan / Masalah</label>
                            <textarea name="pesan" rows="5" class="form-input-premium mt-1.5" placeholder="Tuliskan kendala Anda secara detail di sini agar tim admin dapat membantu..." required>{{ old('pesan') }}</textarea>
                    ...
                    <div class="mt-8 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                        <a href="{{ route('guru.hubungi-admin.index') }}" class="btn-secondary w-full sm:w-auto text-center">Batal</a>
