            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 stagger-children">
                ...
                <div class="stat-card">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-[10px] font-semibold text-surface-400 uppercase tracking-widest">Bimbingan</span>
                    </div>
                ...
                <div class="lg:col-span-2 card-premium p-5 sm:p-6 md:p-8 animate-slide-up" style="animation-delay: 200ms">
                    ...
                    <div class="space-y-3">
                        @if($menungguValidasi > 0)
                            <div class="p-4 bg-amber-50/60 rounded-xl flex items-center justify-between border border-amber-100/60">
                                <div class="flex items-center gap-3">
                                    <div class="w-2 h-2 bg-amber-400 rounded-full animate-pulse"></div>
                                    <span class="text-sm text-amber-800 font-medium">{{ $menungguValidasi }} Pengajuan PKL belum Anda tinjau</span>
                                </div>
                                <a href="{{ route('guru.pengajuan.index') }}" class="text-xs font-bold text-amber-700 hover:underline shrink-0 ml-3">Tinjau →</a>
                            </div>
                        @endif
                        @if($jurnalMenunggu > 0)
                            <div class="p-4 bg-orange-50/60 rounded-xl flex items-center justify-between border border-orange-100/60">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-sm text-orange-800 font-medium">{{ $jurnalMenunggu }} Jurnal harian menunggu validasi</span>
                                </div>
                                <a href="{{ route('guru.jurnal.index') }}" class="text-xs font-bold text-orange-700 hover:underline shrink-0 ml-3">Validasi →</a>
                            </div>
                        @endif
                        @if($laporanMenunggu > 0)
                            <div class="p-4 bg-emerald-50/60 rounded-xl flex items-center justify-between border border-emerald-100/60">
                                <div class="flex items-center gap-3">
                                    <div class="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></div>
                                    <span class="text-sm text-emerald-800 font-medium">{{ $laporanMenunggu }} Laporan akhir menunggu review</span>
                                </div>
                                <a href="{{ route('guru.laporan.index') }}" class="text-xs font-bold text-emerald-700 hover:underline shrink-0 ml-3">Review →</a>
                            </div>
                        @endif
                        @if($penilaianMenunggu > 0)
                            <div class="p-4 bg-cyan-50/60 rounded-xl flex items-center justify-between border border-cyan-100/60">
                                <div class="flex items-center gap-3">
                                    <div class="w-2 h-2 bg-cyan-400 rounded-full animate-pulse"></div>
                                    <span class="text-sm text-cyan-800 font-medium">{{ $penilaianMenunggu }} Siswa menunggu penilaian akhir</span>
                                </div>
                                <a href="{{ route('guru.penilaian.index') }}" class="text-xs font-bold text-cyan-700 hover:underline shrink-0 ml-3">Beri Nilai →</a>
                            </div>
                        @endif
