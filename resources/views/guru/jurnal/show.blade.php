<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('guru.jurnal.index') }}" class="p-2 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-surface-600 transition-colors">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <h2 class="font-extrabold text-lg md:text-2xl text-surface-900 tracking-tight">Validasi Jurnal</h2>
        </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                    <div class="card-premium p-5">
                        ...
                        <div class="flex items-center p-3.5 bg-surface-50 rounded-2xl border border-surface-100">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-surface-400 font-medium">NIS: {{ $jurnalPkl->pengajuanPkl->siswa->nis }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-premium p-5 border-t-4 border-brand-500">
                        ...
                        <div class="space-y-6">
                            <!-- Valid Form -->
                            <form action="{{ route('guru.jurnal.valid', $jurnalPkl) }}" method="POST">
                                @csrf @method('PUT')
                                <input type="text" name="catatan_guru" placeholder="Catatan pujian (opsional)" class="form-input-premium mb-3 !py-2.5 text-sm">
                            ...
                            <!-- Revisi Form -->
                            <form action="{{ route('guru.jurnal.revisi', $jurnalPkl) }}" method="POST">
                                @csrf @method('PUT')
                                <textarea name="catatan_guru" placeholder="Apa yang perlu diperbaiki?" class="form-input-premium mb-3 !py-2.5 text-sm" rows="3" required></textarea>
