                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <select name="tempat_pkl_id" class="form-input-premium mt-1.5 !py-2.5 text-sm" required>
                                ...
                                @foreach($tempatPkl as $t)
                                    <option value="{{ $t->id }}" @selected(old('tempat_pkl_id') == $t->id)>{{ $t->nama_tempat }} ({{ $t->bidang_usaha }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai') }}" class="form-input-premium mt-1.5" required>
                                @error('tanggal_mulai') <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="form-label">Tanggal Selesai</label>
                                <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai') }}" class="form-input-premium mt-1.5" required>
                        ...
                        <div>
                            ...
                            <textarea name="alasan" rows="3" class="form-input-premium mt-1.5" required>{{ old('alasan') }}</textarea>
