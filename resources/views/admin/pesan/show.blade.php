            ...
            <div class="flex flex-col lg:flex-row h-[calc(100vh-14rem)] border border-surface-200 dark:border-zinc-800 rounded-2xl overflow-hidden bg-white dark:bg-zinc-950 shadow-glass animate-fade-in">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <h4 class="text-xs font-bold text-surface-900 dark:text-zinc-100 truncate pr-2"
                                            x-text="c.pembimbing_industri?.user?.name || 'Pembimbing'">
                                        </h4>
                                    ...
                                    <div class="flex items-center justify-between mt-1.5">
                                        <span class="text-[9px] font-semibold px-2 py-0.5 rounded bg-surface-100 dark:bg-zinc-850 text-surface-600 dark:text-zinc-300"
                                              x-text="c.pembimbing_industri?.tempat_pkl?.nama_tempat || 'PKL'">
                                        </span>
                                    </div>
                                </div>
                            </a>
                    ...
                    <div class="px-4 sm:px-6 py-4 bg-white dark:bg-zinc-900 border-b border-surface-200 dark:border-zinc-800 flex items-center justify-between shrink-0 shadow-sm">
                        <div class="flex items-center gap-2 sm:gap-3">
                            ...
                            <div class="w-10 h-10 rounded-full bg-brand-600 text-white flex items-center justify-center font-extrabold text-sm shrink-0"
                                 x-text="(activeConversation.pembimbing_industri?.user?.name || 'P').charAt(0).toUpperCase()">
                            </div>
                            <div>
                                <h3 class="text-sm font-bold text-surface-900 dark:text-zinc-100"
                                    x-text="activeConversation.pembimbing_industri?.user?.name || 'Pembimbing Industri'">
                                </h3>
                ...
                <div :class="activePanel === 'profile' ? 'flex w-full h-full' : 'hidden lg:flex lg:w-72 h-full'" 
                    ...
                    <div class="card-premium p-4 shadow-sm border border-surface-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900">
                        <h4 class="text-xs font-bold text-surface-400 dark:text-zinc-500 uppercase tracking-wider mb-4">Profil Pengirim</h4>
                        
                        <div class="space-y-4">
                            <div>
                                <span class="text-[10px] text-surface-400 dark:text-zinc-500 font-bold uppercase tracking-wider">Nama Pembimbing</span>
                                <p class="text-xs font-bold text-surface-900 dark:text-zinc-100 mt-0.5" x-text="activeConversation.pembimbing_industri?.user?.name || '-'"></p>
                            </div>
                            <div>
                                <span class="text-[10px] text-surface-400 dark:text-zinc-500 font-bold uppercase tracking-wider">Perusahaan / Instansi</span>
                                <p class="text-xs font-bold text-surface-900 dark:text-zinc-100 mt-0.5" x-text="activeConversation.pembimbing_industri?.tempat_pkl?.nama_tempat || '-'"></p>
                            </div>
                            <div>
                                <span class="text-[10px] text-surface-400 dark:text-zinc-500 font-bold uppercase tracking-wider">Jabatan</span>
                                <p class="text-xs text-surface-700 dark:text-zinc-350 mt-0.5" x-text="activeConversation.pembimbing_industri?.jabatan || '-'"></p>
                            </div>
                            <div>
                                <span class="text-[10px] text-surface-400 dark:text-zinc-500 font-bold uppercase tracking-wider">Kontak HP</span>
                                <p class="text-xs text-surface-700 dark:text-zinc-350 mt-0.5" x-text="activeConversation.pembimbing_industri?.no_hp || '-'"></p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        function adminChatRoom(config) {
            return {
                ...
                get fetchUrl() {
                    return `/${this.routePrefix}/pesan-industri/${this.activeId}/messages`;
                },

                get sendUrl() {
                    return `/${this.routePrefix}/pesan-industri/${this.activeId}/balas`;
                },
                ...
                selectConversation(c) {
                    ...
                    const newUrl = `/${this.routePrefix}/pesan-industri/${c.id}`;
