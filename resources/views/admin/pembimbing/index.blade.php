<x-app-layout>
    ...
    <div class="py-6 md:py-10 animate-fade-in">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Search & Filter Form --}}
            <form method="GET" action="{{ route('admin.pembimbing-industri.index') }}" class="mb-5 flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1 max-w-sm">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-surface-400">
                        <i data-lucide="search" class="w-4 h-4"></i>
                    </span>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari pembimbing..." class="form-input-premium !pl-9 !py-2 text-sm w-full">
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(request('search') || request('periode_id'))
                        <a href="{{ route('admin.pembimbing-industri.index') }}" class="btn-secondary !py-2 !px-3 inline-flex items-center justify-center hover:bg-surface-200" title="Reset">
